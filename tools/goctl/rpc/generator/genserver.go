...
func (s *{{.server}}Server) {{.method}} ({{if .notStream}}ctx context.Context,{{if .hasReq}} in {{.request}}{{end}}{{else}}{{if .hasReq}} in {{.request}},{{end}}stream {{.streamBody}}{{end}}) ({{if .notStream}}{{.response}},{{end}}error) {
	l := {{.logicPkg}}.New{{.logicName}}({{if .notStream}}ctx,{{else}}stream.Context(),{{end}}s.svcCtx,rpcLog.NewRpcLogger(map[string]interface{}{"path": {{.serverFullMethod}},
		"uid": ctxdata.GetUidFromCtx(ctx), "ssid": ctxdata.GetSsidFromCtx(ctx), "trace": trace.TraceIDFromContext(ctx)}))
...
func (g *Generator) genServerInCompatibility(ctx DirContext, proto parser.Proto,
	...
	dir := ctx.GetServer()
	traceImports := fmt.Sprintf(`"%v"`, "github.com/zeromicro/go-zero/core/trace")
	...
	imports := collection.NewSet()
	imports.AddStr(traceImports, extendImports, rpcLogImport, logicImport, svcImport, pbImport)
